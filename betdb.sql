...
CREATE TABLE Admin(
   ...
   FOREIGN KEY(id) REFERENCES SUSER(id) ON DELETE CASCADE
);
...
CREATE TABLE Betslip (
           ...
           creator_user_id   smallint NOT NULL REFERENCES SUSER(id) ON DELETE CASCADE,
...
CREATE TABLE League (
           ...
           FOREIGN KEY(branch_name) REFERENCES SportBranch(branch_name) ON DELETE CASCADE);
CREATE TABLE Team (
...
CREATE TABLE Post (
              ...
              no_of_likes        INT,
              post_date            TIMESTAMP,
              FOREIGN KEY(betslip_id) REFERENCES Betslip(betslip_id) ON DELETE CASCADE,
              FOREIGN KEY(user_id) REFERENCES SUSER(id) ON DELETE CASCADE);
CREATE TABLE Comment(
           comment_id        smallserial PRIMARY KEY,
           post_id           smallint NOT NULL,
           commenter_id            smallserial REFERENCES SUSER(id) ON DELETE CASCADE,
...
CREATE TABLE Transaction (
       ...
       sender_player_id    smallint NOT NULL,
           FOREIGN KEY(receiver_player_id) REFERENCES SUSER(id) ON DELETE CASCADE,
       FOREIGN KEY(sender_player_id) REFERENCES SUSER(id) ON DELETE CASCADE);
...
CREATE TABLE UserFollows (
           ...
           FOREIGN KEY(follower_id) REFERENCES SUSER(id) ON DELETE CASCADE,
           FOREIGN KEY(followee_id) REFERENCES SUSER(id) ON DELETE CASCADE,
...
where t1.team_id = home_team_id and t2.team_id = away_team_id and cancelled = 0
