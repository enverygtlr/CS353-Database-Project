...
def follow_user(user_id = '4', requested_user_id = '10'):
    conn = connectToPostgres()
    ...
    if len(data) == 0:
        ...
        if user_type[0]["s_type"] == "editor":
            execute("update editor set no_of_followers = no_of_followers + %s  where id = %s " , conn ,select=False  , args=([ '1' , requested_user_id]))
        elif user_type[0]["s_type"] == "player":
            execute("update player set no_of_followers = no_of_followers + %s  where id = %s ", conn , select=False , args=([ '1' , requested_user_id]))
        return True
    else:
        return False
...
def play_bet(list_bets = [{'team1':'fenerbahce', 'team2':'bursa', 'date':'2019-02-19 00:00:00', 'type':'1.5 ALT'}], user_id = 5, stake = 10, shared= 0):
    ...
    execute("update betslip set total_odd = %s where betslip_id  = %s " , conn , select = False ,args = (total_odd , betslip_id[0]['betslip_id']))
...
def isbetslipSuccesful(betslip_id = '1'):
    ...
    for i in range(len(betslip)):
       ...
       if isBetSuccessful(bet_id,branch_name[0]["branch_name"]) != True: #valla size birakiyorum karari bu fonksiyon boolean mi dondursun falan filan, duruma
            return False
       print(bet_id)
       print(branch_name[0]["branch_name"])
    return True
